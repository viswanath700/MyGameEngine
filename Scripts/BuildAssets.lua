...
do
	...
	do
		...
		local errorMessage
		s_AuthoredAssetDir, errorMessage = LuaGetEnvironmentVariable( key )
	...
	do
		...
		local errorMessage
		s_BuiltAssetDir, errorMessage = LuaGetEnvironmentVariable( key )
	...
	do
		local key = "BinDir"
		local errorMessage
		s_BinDir, errorMessage = LuaGetEnvironmentVariable( key )
...
local function BuildAsset( i_sourceRelativePath, i_targetRelativePath, i_builderFileName, i_dependencyList )
	...
	do
		...
		if doesTargetExist then
			...
			local lastDependencyWriteTimePrevious = 0
			
			for dependencyKey, dependencyValue in ipairs(i_dependencyList) do
				if dependencyValue then 
					local dependencySource = s_AuthoredAssetDir .. dependencyValue
					lastDependencyWriteTime = math.max(GetLastWriteTime(dependencySource), lastDependencyWriteTimePrevious)
					lastDependencyWriteTimePrevious = GetLastWriteTime(dependencySource)
				else
	...
	print("building:" .. i_builderFileName);
		...
		print("Not building:" .. i_builderFileName);
		return true
	end
end
...
local function BuildAssets( i_assetsToBuild )
	...
	for assetType, assetInfo_singleType in pairs( i_assetsToBuild ) do
		...
		local sourcePathName = assetInfo_singleType.sourcePath
		local targetPathName = assetInfo_singleType.targetPath
		
		local sourceExtensionName = assetInfo_singleType.sourceExtension
		local targetExtensionName = assetInfo_singleType.targetExtension
		
		local assets = assetInfo_singleType.assets
		
		local dependencyList = assetInfo_singleType.dependencies
		
		for j, assetInfo in ipairs( assets ) do
			if not BuildAsset(sourcePathName..assetInfo..sourceExtensionName,
							  targetPathName..assetInfo..targetExtensionName,
							  builderFileName,
							  dependencyList) then
