	...
	namespace Graphics
	{
		...
		void CollisionDetection::LoadCollisionData(const char* meshPath)
		{
			...
			if (inputBinaryMeshFile) {
				...
				inputBinaryMeshFile.read(binaryReadBuffer, fileLength);

				// Extracting corresponding info after reading binary
				m_vertexCount = *reinterpret_cast<uint32_t*>(binaryReadBuffer);
				m_vertexData = reinterpret_cast<Mesh::sVertex*>(binaryReadBuffer + 4 * 2);

				m_indexCount = *reinterpret_cast<uint32_t*>(binaryReadBuffer + 4);
				m_indexData = reinterpret_cast<uint32_t*>(binaryReadBuffer + 4 * 2 + sizeof(Mesh::sVertex) * m_vertexCount);
			}
